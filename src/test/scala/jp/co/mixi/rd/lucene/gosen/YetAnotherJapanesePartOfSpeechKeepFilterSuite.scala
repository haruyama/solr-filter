...
class YetAnotherJapanesePartOfSpeechKeepFilterSuite extends FilterSuite {
  ...
  test("形容詞1") {
    ...
    val stream = new YetAnotherJapanesePartOfSpeechKeepFilter(new JapaneseTokenizer(reader), "形容詞")
  ...
  test("名詞1") {
    val reader = new StringReader("かわいい服")
    val stream = new YetAnotherJapanesePartOfSpeechKeepFilter(new JapaneseTokenizer(reader), "名詞")
